...
public class ClusterEntity {
    ...
    public ClusterEntity(int TingkatKematianCentroid, int JumlahKasusCentroid, 
            int PotensiPenularan, int clusterNumber){
        super();
    ...
    public double calculateDistance(RecordEntity record){
        return Math.sqrt(Math.pow((getTingkatKematianCentroid() - record.getTingkatKematian()),2) + 
                Math.pow((getJumlahKasusCentroid() - record.getJumlahKasus()),2) + Math.pow
                ((getPotensiPenularan() - record.getPotensiPenularan()),2));
    }
